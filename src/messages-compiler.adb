...
package body Messages.Compiler is
   ...
   begin
      ...
      Stop_Index := Messages.Compiled.Last_Index (Build);
   end Append_Stop;
   ...
   begin
      Jump.Kind := Messages.Compiled.Jump_Plural;
      ...
      if Exact_Count > 0 then
         declare
            ...
            Branch        : Messages.AST.Branch_Access := Node.Plural_Exact;
   ...
   begin
      Jump.Kind := Messages.Compiled.Jump_Ordinal;
      Jump.Key := new String'(To_String (Node.Name));
      Messages.Compiled.Append (Build, Jump);
      Jump_Index := Messages.Compiled.Last_Index (Build);
      ...
      if Exact_Count > 0 then
         declare
            Exact_Targets : Messages.Compiled.Exact_Target_Array (1 .. Exact_Count);
            Exact_Stops   : array (1 .. Exact_Count) of Natural := [others => 0];
            Branch        : Messages.AST.Branch_Access := Node.Ord_Exact;
            Index         : Positive := Exact_Targets'First;
         begin
            while Branch /= null loop
               Exact_Targets (Index).Value :=
